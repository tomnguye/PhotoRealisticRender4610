...
public:
    ...
    static Vector3f Min(const Vector3f &p1, const Vector3f &p2) { return Vector3f(std::min(p1.x, p2.x), std::min(p1.y, p2.y), std::min(p1.z, p2.z)); }

    static Vector3f Max(const Vector3f &p1, const Vector3f &p2) { return Vector3f(std::max(p1.x, p2.x), std::max(p1.y, p2.y), std::max(p1.z, p2.z)); }
};
...
inline Vector3f power(const Vector3f &a, float exp) { return Vector3f(powf(a.x, exp), powf(a.y, exp), powf(a.z, exp)); }
